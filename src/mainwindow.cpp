...
void MainWindow::initialMIBTreeWidget(QTreeWidget* MIBTreeWidget) {
  ...
  MIBTreeWidget->header()->setSectionResizeMode(QHeaderView::ResizeToContents);
...
void MainWindow::initialResultTableWidget(QTableWidget* resultTableWidget) {
  ...
  resultTableWidget->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
}
...
void MainWindow::initialConnections() {
  ...
  connect(ui->actionAbout_Qt, SIGNAL(triggered()), QApplication::instance(),
          SLOT(aboutQt()));
  connect(ui->actionAbout_this, SIGNAL(triggered()), this, SLOT(aboutThis()));
  /*Initialize Connections between Widgets*/
  connect(ui->IPLineEdit, SIGNAL(textChanged(QString)), this,
          SLOT(onIPLineEditChanged(QString)));
  connect(ui->OidLineEdit, SIGNAL(textChanged(QString)), this,
          SLOT(onOidLineEditChanged(QString)));
  connect(ui->advancedOptionPushButton, SIGNAL(clicked()), this,
          SLOT(onAdvancedOptionPushButtonClicked()));
  connect(ui->goPushButton, SIGNAL(clicked()), this,
          SLOT(onGoPushButtonClicked()));
  /*connect actions*/
  connect(ui->actionClear, SIGNAL(triggered()), this,
          SLOT(resetResultTableWidget()));
  connect(ui->actionLoad_MIB, SIGNAL(triggered()), this, SLOT(loadMIB()));
  connect(this, SIGNAL(statusBarUpdate(QString)), ui->statusBar,
          SLOT(showMessage(QString)));
  // TODO
  /*ResultTable*/
  connect(ui->resultTableWidget, SIGNAL(cellClicked(int, int)), this,
          SLOT(onResultTableClicked(int, int)));
  connect(ui->resultTableWidget, SIGNAL(cellDoubleClicked(int, int)), this,
          SLOT(resultTableDetail(int, int)));
  /*connect MIBTree*/
  connect(ui->MIBTreeWidget, SIGNAL(itemClicked(QTreeWidgetItem*, int)), this,
          SLOT(onTreeItemClicked(QTreeWidgetItem*)));
  /*connect quick actions*/
  connect(ui->actionGet, SIGNAL(triggered()), this, SLOT(onActionGet()));
  connect(ui->actionGetNext, SIGNAL(triggered()), this,
          SLOT(onActionGetNext()));
...
Status MainWindow::setUpRequestSet(Request* request) {
  ...
  SetRequestDialog* dialog =
      new SetRequestDialog(&(request->data), node->syntax, this);
  dialog->exec();
  if (dialog->cancel) {
...
void MainWindow::aboutThis() {
  QMessageBox::about(
      ...
      tr("The Application is based on <a "
         "href=\"http://www.agentpp.com/\"><b>snmp++</b></a>"
